			...
			<div className={classes.taskLine}>
				<div className={classes.taskDescription}>
					<Checkbox 
						...
						onChange={updateStatusTaskHandle} 
					/>
					{isEdit ? (
						<Input 
				...
				<div className={classes.icons}>
					{isEdit ? (
						<DoneIcon style={{cursor: 'pointer'}} onClick={updateDescriptionTaskHandler} />
					) : (
						<EditIcon style={{cursor: 'pointer'}} onClick={() => setIsEdit(true)} />
					)}
					<ClearIcon style={{cursor: 'pointer'}} onClick={()=>{task.id && deleteTask(task.id)}} />	
				</div>
